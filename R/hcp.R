...
cor_w_pop_by_region <- function(
  ...
  tfce <- tfce |>
    dplyr::filter(stringr::str_detect(
      tfce_corrp_tstat,
      glue::glue("flags-{ContrastName}_tfce")
    ))
  tfce_pop <- tfce_pop |>
  ...
  study <- get_pairs(
    fs::path(storage_dir, fs::path_file(tfce$tstat)),
    tfce$n_sub
  ) |>
    mask() |>
    mask_gray() |>
  ...
  test <- get_pairs(
    fs::path(storage_dir, fs::path_file(tfce_pop$tstat)),
    tfce_pop$n_sub
  ) |>
    mask() |>
    mask_gray() |>
  ...
  dplyr::left_join(study, test, by = c("x", "y", "z")) |>
    ...
    dplyr::summarise(
      rho = cor(study, test, method = .env$method),
      .groups = "drop"
    ) |>
    dplyr::bind_cols(
      tfce |>
...
get_pairs <- function(tstat, n_sub) {
  cope <- to_tbl(
    stringr::str_replace(tstat, "_tstat1", "_glm_cope"),
    measure = "cope"
  )
  varcope <- to_tbl(
    stringr::str_replace(tstat, "_tstat1", "_glm_varcope"),
    measure = "varcope"
  ) |>
...
get_hcp_copes <- function(
  ...
  hcp_copes <- tibble::tibble(
    msmall_feat = fs::path(readr::read_lines(msmall_feat_file))
  ) |>
    dplyr::mutate(
    ...
    dplyr::rowwise() |>
    dplyr::mutate(
      MSMALL = fs::path_join(c(
        msmall_feat,
        "GrayordinatesStats",
        glue::glue("cope{CopeNumber}.feat")
      )),
    ) |>
    dplyr::ungroup() |>
    dplyr::select(-msmall_feat) |>
    dplyr::mutate(
      VOL = stringr::str_replace(
        MSMALL,
        "_MSMAll.feat/GrayordinatesStats/",
        "vol.feat/"
      ),
...
get_hcp_contrasts <- function(
  src = here::here("data-raw", "hcp", "contrasts.csv")
) {
