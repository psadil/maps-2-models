fsl_randomise <- function(input, output, ...) {
  ...
  fs::dir_ls(
    fs::path_dir(output),
    regexp = paste0(fs::path_file(output), "_.*(tfce|tstat1).*")
  )
}
...
do_tfce2 <- function(
  ...
  filestem <- fs::path(
    storage_dir,
    glue::glue("nsub-{n_sub}_iter-{iter}_flags-{flags}")
  )
  out <- fsl_randomise(
    merged,
    filestem,
...
.get_tfce_cmd <- function(
  ...
  stem <- glue::glue(
    "nsub-{n_sub}_iter-{iter}_task-{Task}_cope-{CopeNumber}_type-{type}"
  )
  prefix <- fs::path(Sys.getenv("NIIDIR"), stem)

  if (type %in% c("VOL", "UKB")) {
    cmd <- .data |>
      dplyr::rename(value = tidyselect::all_of(type)) |>
...
get_tfce_pop <- function(test, n = 1) {
  test |>
    tidyr::pivot_longer(
      tidyselect::any_of(c("MSMALL", "SURFACE", "VOL", "UKB")),
      names_to = "type"
    ) |>
    dplyr::distinct(Task, CopeNumber, type, sub) |>
    dplyr::mutate(
      n_sub = dplyr::n_distinct(sub),
      .by = c(Task, CopeNumber, type)
    ) |>
    dplyr::mutate(
...
.get_palm_maxes_vol <- function(samples, cluster_thresh = 0.0001) {
  ...
  tstat <- fs::path(
    Sys.getenv("NIIDIR"),
    glue::glue("{filestem}_tfce_tstat_c1.nii")
  )
  fwep <- fs::path(
    Sys.getenv("NIIDIR"),
    glue::glue("{filestem}_tfce_tstat_fwep_c1.nii")
  )
