...
::: {.bullets}
- Decades of fMRI research on brain-behavior relationships are obscured by small samples (Figure 1; Marek et al. 2022)
- Large-scale studies provide a 
::: 
...
to_tbl(tfce_pop$tstat[[1]]) |>
  ...
  geom_point(color="red", data=distinct(space, x,y,z), size=5) +
...
```{r, fig.width=20, fig.height=18, fig.cap=""}

a <- targets::tar_read(rho_quants, store=params$tfce_store) |>
  ggplot(aes(x=n_sub)) +
  geom_boxplot(
